...
create_vaccine_coverage_routine_sia <- function (vaccine_coverage_folder    = "",
  ...
  if ((rev_cov == TRUE) & (nrow(sia) > 0)) {
    ...
    sia$age_range_verbatim2 <- tolower (gsub("[[:space:]]", "", sia$age_range_verbatim))
    sia [age_range_verbatim2 == "defaultageandgender" | is.na(age_range_verbatim2),
         age_range_verbatim2 := paste0 (age_first, "-", age_last, "y")]

    # adjust format for following extraction of precise age (update for 2021 version)
    sia [age_range_verbatim2 == ">12yoroutsidetargetvaccinated" , age_range_verbatim2 := "12-100y"]
    sia [age_range_verbatim2 == "16-35yfemales", age_range_verbatim2 := "16-35y"]
    sia [age_range_verbatim2 == "17-24ymales", age_range_verbatim2 := "17-24y"]
    sia [age_range_verbatim2 == "<5y", age_range_verbatim2 := "0-4y"]
    sia [age_range_verbatim2 == "<15y", age_range_verbatim2 := "0-14y"]
    sia [age_range_verbatim2 == "6m+", age_range_verbatim2 := "6m-100y"]
    sia [age_range_verbatim2 == "6m-<6y", age_range_verbatim2 := "6m-5y"]
    sia [age_range_verbatim2 == "6m-<15y", age_range_verbatim2 := "6m-14y"]
    sia [age_range_verbatim2 == "6m-9yo", age_range_verbatim2 := "6m-9y"]
    sia [age_range_verbatim2 == "9m-<5y", age_range_verbatim2 := "9m-4y"]
    sia [age_range_verbatim2 == "9m-<13y", age_range_verbatim2 := "9m-12y"]
    sia [age_range_verbatim2 == "9m-<15y", age_range_verbatim2 := "9m-14y"]
    sia [age_range_verbatim2 == "school-age", age_range_verbatim2 := "6y-17y"]
    sia [age_range_verbatim2 == "1yschool", age_range_verbatim2 := "4y"]

    sia [, `:=`(age_first_verb = stringr::str_extract (age_range_verbatim2, "[^-]+"),
                age_last_verb  = stringr::str_extract (age_range_verbatim2, "[^-]+$"))]
    ...
    sia [age_first_unit == "y" ,
         age_first := age_first_num]
    sia [age_last_unit == "m" ,
         age_last := ifelse (age_last_num < 36, age_last_num/12, round(age_last_num/12))]
    sia [age_last_unit == "y" ,
         age_last := ifelse (age_last_num <=3, age_last_num*12, age_last_num)]
    ...
    sia [age_range_verbatim %in% c("16-35y Females", "17-24y Males"),
          target2 := target]
    ...
    sia <- sia [, .SD, .SDcols = !c("age_first_verb", "age_last_verb",
                                    ...
                                    "target2", "coverage2", "age_range_verbatim2",
                                    "age_range_verbatim", "reached")]
    }
...
#' sensitivity analysis (PSA). Seven variables are included in the output: (1)
...
#' (3) "take3_input" - vaccine protection for two doses under 'take' assumption,
#' (4) "R0_input" - basic reproduction number, (5) "cfrpast_input" - difference
#' in country-specific case fatality risks (CFRs) prior to 2015, (6)
#' "cfrproj_input" - difference in country-specific CFRs after 2015, and (7)
#' "siadose_input" - the method for distributing doses between zero-dose and
#' already-vaccinated populations in supplementary immunisation activities (SIAs).
#'
#' @param psa An integer indicating the total runs of PSA.
#' @param seed_state An integer for selecting the seed number.
#' @param psadat_filename A file name containing iteration ID and input
#' parameters of each run of PSA.
#'
#' @import data.table
#'
#' @examples
...
create_PSA_data <- function (psa             = 0,
  ...
  # intercept of regression function for first-dose vaccine efficacy by age
  psadat [, vaceffbyage_a := 0]

  # slope of regression function for first-dose vaccine efficacy by age
  ...
  # difference in country-specific case fatality rate
  psadat [, cfrpast_input := 0] # before 2015
  psadat [, cfrproj_input := 0] # after 2015

  #  distribution of SIA doses to zero-dose and already-vaccinated populations
  psadat [, siadose_input := 0]
  # ----------------------------------------------------------------------------
  ...
  #                                                 )
  #         ]
  # update 2021: beta distribution based on Sudfeld et al. (2010)
  psadat [, take3_input := qbeta (cube [, 1], 18.6240930, 0.3106631)]

  # ----------------------------------------------------------------------------
  # R0
  # lognormal distribution based on Guerra et al. (2017)
  psadat [, R0_input := EnvStats::qlnormTrunc (cube [, 2],
                                               2.5763119,
                                               0.5363589,
                                               min = 3.775582,  #  1% quantile
                                               max = 45.79016   # 99% quantile
                                               )
          ]
  ...
  # psadat [, mortality_input := truncnorm::qtruncnorm (cube [, 3],
  #                                                     a    = (1 - 0.25),
  #                                                     b    = (1 + 0.25),
  #                                                     mean = 1,
  #                                                     sd   = (0.25/3)
  #                                                     )
  #         ]
  # update 2021: truncated normal distributions based on Portnoy et al. (2018)
  # past and projection periods separated by 2015
  # parameterise by using mean CFRs estimates according to CLT (from 124 studies)
  cfr_past_se = 0.009706518/sqrt(124)
  cfr_proj_se = 0.008706414/sqrt(124)
  psadat [, cfrpast_input := truncnorm::qtruncnorm (cube [, 3],
                                                    a    = - 3*cfr_past_se,
                                                    b    =   3*cfr_past_se,
                                                    mean = 0,
                                                    sd   = cfr_past_se)
          ]
  psadat [, cfrproj_input := truncnorm::qtruncnorm (cube [, 3],
                                                    a    = - 3*cfr_proj_se,
                                                    b    =   3*cfr_proj_se,
                                                    mean = 0,
                                                    sd   = cfr_proj_se)
  ...
  # ----------------------------------------------------------------------------
  # distribution of SIA doses
  # "portnoy" - weighted logistic function based on Portnoy's data
  # "nreach" - 7.7% of children are never reached for measles vaccination
  # psadat [, siadose_input := ifelse ( rbinom(psa,1,.5) == 1, "portnoy", "nreach")]
  psadat [, siadose_input := ifelse (cube [, 4] < 0.5, "portnoy", "nreach")]

...
plt_data <- rbind (outfile_mcv1_mcv2_sia,
                   copy (outfile_mcv1_mcv2alt [vaccine == "MCV2"])[, vaccine := "MCV2 \n(early intro)"])
...
pdf ("plot/fig1_coverage.pdf", width = 14, height = 7)
...
# figure Sx: MCV2, MCV2 (early intro) coverage
pdf ("plot/figs1_mcv2-alt-cov.pdf", width = 12, height = 6)
plt_mcv2_cov <- ggplot (data = plt_data [vaccine != "SIAs" & year >= 2000],
                   aes (x = year, y = coverage, colour = vaccine, linetype = vaccine)) +
  scale_x_continuous (breaks = pretty_breaks ()) +
  geom_line (size = 0.9) +
  facet_wrap (vars(country), ncol = 5) +
  labs (title = " ", x = "Year", y = "Vaccine coverage") +
  scale_colour_manual ("Delivery strategy", values = c("#42b540", "#00468b", "#0099b4")) +
  scale_linetype_manual ("Delivery strategy", values = c(1,1,2)) +
  theme_bw() +
  theme (legend.position  = c(0.9, 0.08),#"bottom"
         legend.direction = "vertical",
         legend.key.size = unit (1, 'cm'),
